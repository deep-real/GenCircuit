...
model_dict = {'ViT-B_16-in21k':'vit_base_patch16_224_in21k',
...
class Result:
    ...
    def output_result(self):
        ...
        wb_group_train_sizes = np.array([3498, 184, 56, 1057]).astype(float)
        wb_group_train_sizes /= wb_group_train_sizes.sum()

        def get_base_mean_acc(accs):
            return (np.array([accs[0], accs[1],
                              accs[2], accs[3]]) * wb_group_train_sizes).sum()

        mean_acc = get_base_mean_acc(accs)
        print(f"test mean accuracy: {mean_acc}")
...
def calculate_acc(args):
    ...
    return result_test.output_result()
