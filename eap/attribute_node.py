...
def get_scores_eap(model: HookedTransformer, graph: Graph, dataloader:DataLoader, metric: Callable[[Tensor], Tensor], 
                   intervention: Literal['patching', 'zero', 'mean','mean-positional', 'optimal']='patching', device='cuda',
                   intervention_dataloader: Optional[DataLoader]=None, optimal_ablation_path: Optional[str] = None, 
                   quiet:bool=False, neuron:bool=False, task=None, model_name=None, model_id=None, profile_one_batch=False):
    ...
    for clean, corrupted, label in dataloader:
        ...
        if is_first_batch and profile_one_batch:
            print("\n" + "="*80)
            print("PROFILING FIRST BATCH (EAP - Node) - Starting batch processing...")
            print("="*80)
            profiler.enable()
        ...
        if is_first_batch and profile_one_batch:
            ...
            print(f"  Data preparation (tokenization): {(t1-t0)*1000:.2f} ms")

        if is_first_batch and profile_one_batch:
            t2 = time.perf_counter()

        (fwd_hooks_corrupted, fwd_hooks_clean, bwd_hooks), activation_difference = make_hooks_and_matrices(model, graph, batch_size, (model.cfg.image_size // model.cfg.patch_size)**2+1, scores, neuron=neuron)
        ...
        if is_first_batch and profile_one_batch:
            ...
            if task is not None and model_id is not None:
                ...
                method_suffix = "neuron" if neuron else "node"
                profile_filename = f"batch_profile_EAP_{method_suffix}_{task.replace('_', '-')}_model{model_id}.txt"
                profile_output_path = os.path.join(profile_dir, profile_filename)
                
                with open(profile_output_path, 'w') as f:
                    f.write("="*80 + "\n")
                    f.write(f"BATCH PROFILING STATISTICS (EAP - {method_suffix.upper()})\n")
                    f.write("="*80 + "\n\n")
                    f.write(f"Task: {task}\n")
                    f.write(f"Model ID: {model_id}\n")
                    f.write(f"Batch size: {batch_size}\n")
                    f.write(f"Neuron-level: {neuron}\n")
...
def get_scores_eap_ig(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metric: Callable[[Tensor], Tensor], 
                      steps=30, quiet:bool=False, neuron:bool=False, task=None, model_name=None, model_id=None, profile_one_batch=False):
    ...
    for clean, corrupted, label in dataloader:
        ...
        if is_first_batch and profile_one_batch:
            print("\n" + "="*80)
            print("PROFILING FIRST BATCH (EAP-IG-inputs - Node) - Starting batch processing...")
            print("="*80)
            profiler.enable()
            batch_start_time = time.perf_counter()
        ...
        for step in range(1, steps+1):
            ...
            if is_first_batch and profile_one_batch:
                ...
                if step == 1 or step == steps or (step) % max(1, steps // 5) == 0:
                    print(f"    IG step {step}/{steps}: {(step_end-step_start)*1000:.2f} ms")
        ...
        if is_first_batch and profile_one_batch:
            ...
            if task is not None and model_id is not None:
                ...
                os.makedirs(profile_dir, exist_ok=True)
                method_suffix = "neuron" if neuron else "node"
                profile_filename = f"batch_profile_EAP-IG-inputs_{method_suffix}_{task.replace('_', '-')}_model{model_id}.txt"
                profile_output_path = os.path.join(profile_dir, profile_filename)
                
                with open(profile_output_path, 'w') as f:
                    f.write("="*80 + "\n")
                    f.write(f"BATCH PROFILING STATISTICS (EAP-IG-inputs - {method_suffix.upper()})\n")
                    f.write("="*80 + "\n\n")
                    f.write(f"Task: {task}\n")
                    f.write(f"Model ID: {model_id}\n")
                    f.write(f"Batch size: {batch_size}\n")
...
def attribute_node(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metric: Callable[[Tensor], Tensor], 
                   ...
                   optimal_ablation_path: Optional[str] = None, quiet:bool=False, neuron:bool=False, device='cuda',
                   task=None, model_name=None, model_id=None, profile_one_batch=False):
    ...
    if method == 'EAP':
        scores = get_scores_eap(model, graph, dataloader, metric, intervention=intervention, 
                                intervention_dataloader=intervention_dataloader, 
                                optimal_ablation_path=optimal_ablation_path, quiet=quiet, neuron=neuron, device=device,
                                task=task, model_name=model_name, model_id=model_id, profile_one_batch=profile_one_batch)
