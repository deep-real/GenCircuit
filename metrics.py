...
def get_metric(metric_name: str, 
    ...
    if metric_name == 'kl_divergence' or metric_name == 'kl':
        ...
        elif model.cfg.n_classes == 2:
            logit_diff_fn = logit_diff
        else:
            logit_diff_fn = logit_diff_multi
        return partial(logit_diff_fn, prob=prob)
    elif metric_name == 'psudo_logit_diff_multi':
        logit_diff_fn = psudo_logit_diff_multi
        return partial(logit_diff_fn, prob=False)
...
def divergence(circuit_logits: torch.Tensor, clean_logits: torch.Tensor, labels: torch.Tensor, divergence_type: Union[Literal['kl'], Literal['js']]='kl', mean=True, loss=True):
...
def logit_diff_binary(circuit_logits: torch.Tensor, clean_logits: torch.Tensor, labels: torch.Tensor, mean=True, prob=False, loss=False):
...
def delta_logit_diff(circuit_logits: torch.Tensor, clean_logits: torch.Tensor, labels: torch.Tensor, mean=True, prob=False, loss=False):
...
def logit_diff(circuit_logits: torch.Tensor, clean_logits: torch.Tensor, labels: torch.Tensor, mean=True, prob=False, loss=False):
    circuit_outputs = torch.softmax(circuit_logits, dim=-1) if prob else circuit_logits
    # good_bad = torch.gather(circuit_outputs, -1, labels.to(circuit_outputs.device))
    labels = torch.tensor(labels, dtype=torch.long, device=circuit_outputs.device)
    good_bad = torch.gather(circuit_outputs, -1, labels) 
...
def logit_diff_multi(circuit_logits: torch.Tensor, clean_logits: torch.Tensor, labels: torch.Tensor, mean=True, prob=False, loss=False):
    circuit_outputs = torch.softmax(circuit_logits, dim=-1) if prob else circuit_logits
    # good_bad = torch.gather(circuit_outputs, -1, labels.to(circuit_outputs.device))
    good_labels = torch.tensor(
        [pair[0].item() for pair in labels],
        dtype=torch.long,
        device=circuit_outputs.device
    )
    good_logits = circuit_outputs.gather(1, good_labels.unsqueeze(1)).squeeze(1)

    # Compute sum of all logits, then subtract good_logits to get sum of other logits
    total_logits = circuit_outputs.sum(dim=1)  # shape [batch_size]
    other_logits_sum = total_logits - good_logits

    # Compute difference
    results = good_logits - other_logits_sum
    if loss:
        # remember it's reversed to make it a loss
        results = -results
    if mean:
...
def psudo_logit_diff_multi(circuit_logits: torch.Tensor, clean_logits: torch.Tensor, labels: torch.Tensor, mean=True, prob=False, loss=False):
    circuit_outputs = torch.softmax(circuit_logits, dim=-1) if prob else circuit_logits
    clean_pred_class = torch.argmax(clean_logits, dim=-1)
    # good_bad = torch.gather(circuit_outputs, -1, labels.to(circuit_outputs.device))
    psudo_good_logits = circuit_outputs.gather(1, clean_pred_class.unsqueeze(1)).squeeze(1)

    # Compute sum of all logits, then subtract good_logits to get sum of other logits
    total_logits = circuit_outputs.sum(dim=1)  # shape [batch_size]
    other_logits_sum = total_logits - psudo_good_logits

    # Compute difference
    results = psudo_good_logits - other_logits_sum
    if loss:
        # remember it's reversed to make it a loss
        results = -results
    if mean:
