...
def evaluate_area_under_curve(model: HookedTransformer, graph: Graph, dataloader, metrics, quiet:bool=False, 
                              level:Literal['edge', 'node','neuron']='edge', log_scale:bool=False, absolute:bool=True, task=None, model_id=None,
                              intervention: Literal['patching', 'zero', 'mean','mean-positional']='patching', intervention_dataloader:DataLoader=None,
                              optimal_ablation_path:Optional[str]=None, no_normalize:Optional[bool]=False, apply_greedy:bool=False, device='cuda'):
    baseline_score = 0 if 'divergence_type' in metrics.keywords.keys() else evaluate_baseline(model, dataloader, metrics, device=device).mean().item()
    graph.apply_topn(0, True)
    corrupted_score = evaluate_graph(model, graph, dataloader, metrics, quiet=quiet, intervention=intervention, intervention_dataloader=intervention_dataloader, task=task, model_id=model_id, device=device).mean().item()
    ...
    for pct in percentages:
        ...
        if apply_greedy:
            ...
            this_graph.apply_topn(curr_num_items, absolute, level=level, prune=False)
        ...
        ablated_score = evaluate_graph(model, this_graph, dataloader, metrics,
                                       ...
                                       intervention_dataloader=intervention_dataloader, task=task, model_name=model_name, model_id=model_id, device=device).mean().item()
        if no_normalize:
...
if __name__ == "__main__":
    csv_path = "/home/yxpengcs/PycharmProjects/vit-spurious-robustness/output/terra-incognita-38_sweep_results.csv"
    import pandas as pd

    get_perexample_scores = True
    circuit_dir = 'circuits'
    fragment = None
    batch_size = 30
    metric_name = 'kl_divergence'
    eval_metric_name = 'kl_divergence'
    method = 'EAP-IG-inputs'
    ablation = 'mean-positional'
    eval_ablation = 'labeled-mean-positional'
    ig_steps = 5
    optimal_ablation_path = None
    device = 'cuda:7'
    level = 'edge'
    df = pd.read_csv(csv_path)
    task = 'terra-incognita-38-mean-location46'
    ...
    for row in df.itertuples(index=False):
        ...
        #     continue
        dataset, intervention_dataset = setup_dataset(task, split=split, model_name=model_name)
        ...
        metric = get_metric(eval_metric_name, task, model, model)
        ...
        if is_ep_direct_edge:
            ...
            method_name_saveable = f"{method}_{ablation}_{level}_train_{metric_name}"
            if center:
                center_str = '_center0'
            else:
                center_str = ''
            if method == 'EP' or method == 'UGS':
                file_name = f'graph{center_str}.json'
            else:
                file_name = 'importances.pt'
            p = f"{circuit_dir}/{method_name_saveable}/{task.replace('_', '-')}_sweep_{model_id}/{file_name}"
            ...
            if random:
                graph.scores = torch.rand_like(graph.scores)

            eval_auc_outputs = evaluate_area_under_curve(model, graph, dataloader, attribution_metric, level=level, task=task, model_id=model_id,
                                                         absolute=absolute, intervention=eval_ablation, intervention_dataloader=intervention_dataloader,
                                                         optimal_ablation_path=optimal_ablation_path, apply_greedy=apply_greedy, device=device)
            weighted_edge_counts, percentages, area_under, area_from_1, average, faithfulnesses = eval_auc_outputs
            ...
            is_greedy = '_greedy' if apply_greedy else ''
            method_name_saveable = f"{method}_{ablation}_{level}{is_greedy}_{metric_name}"
        output_path = os.path.join(output_dir, method_name_saveable)
        os.makedirs(output_path, exist_ok=True)
        random_str = '_rand' if random else ''
        with open(f"{output_path}/{task}_sweep_{model_id}_{split}_abs-{absolute}{center_str}_eval_{eval_ablation}_{eval_metric_name}{random_str}.pkl", 'wb') as f:
            pickle.dump(d, f)
        ...
        # Adjust layout
        plt.tight_layout()
        plt.savefig(f"{output_path}/{task}_sweep_{model_id}_{split}_abs-{absolute}{center_str}_eval_{eval_ablation}_{eval_metric_name}{random_str}.png")