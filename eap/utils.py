...
def make_hooks_and_matrices(model: HookedTransformer, graph: Graph, total_items: int, batch_size:int , n_pos:int, scores: Optional[Tensor], per_example_scores=None):
    ...
    def gradient_hook(prev_index: int, bwd_index: Union[slice, int], gradients:torch.Tensor, hook):
        ...
        try:
            ...
            if per_example_scores is not None:
                per_example_scores[total_items: total_items + batch_size, :prev_index, bwd_index] += s.squeeze(2).detach().cpu()
        except RuntimeError as e:
...
def compute_mean_activations(model: HookedTransformer, graph: Graph, dataloader: DataLoader, per_position=False, device='cuda'):
    """
    ...
    for batch in tqdm(dataloader, desc='Computing mean'):
        ...
        if not means_initialized:
            ...
            if per_position:
                means = torch.zeros(((model.cfg.image_size // model.cfg.patch_size)**2+1, graph.n_forward, model.cfg.d_model), device=device, dtype=model.cfg.dtype)
            else:
...
def compute_mean_activations_per_class(model, graph, dataloader, num_classes, per_position=False, device='cuda'):
    """
    Compute the sum of activations for each class and count how many samples per class.
    Returns:
        class_activations: dict[class_id][node_index] -> activation sum (Tensor)
        class_counts: dict[class_id] -> number of samples
    """

    def activation_hook(index, activations, hook, class_id=None, per_class_acts=None):
        acts = activations.detach()

        for i, cls in enumerate(class_id):
            per_class_acts[cls.item()][:, index] += acts[i]
    ...
    if per_position:
        class_activations = defaultdict(
            lambda: torch.zeros(((model.cfg.image_size // model.cfg.patch_size) ** 2 + 1, graph.n_forward, model.cfg.d_model), device=device, dtype=model.cfg.dtype)
        )
    else:
        class_activations = defaultdict(
            lambda: torch.zeros((graph.n_forward, model.cfg.d_model), device=device, dtype=model.cfg.dtype)
        )
    class_counts = defaultdict(int)

    for batch in tqdm(dataloader, desc="Computing class-wise activations"):
        batch_inputs, _, labels = batch
        batch_inputs = torch.stack(batch_inputs).to(device)
        correct_labels = tuple(lst[0] for lst in labels)

        with torch.no_grad():
            pred_logits = model(batch_inputs)
            pred_classes = torch.argmax(pred_logits, dim=-1)

        hooks = [
            (hook_point, partial(activation_hook, index, class_id=pred_classes, per_class_acts=class_activations))
        ...
        for pred_class in pred_classes:
            class_counts[pred_class.item()] += 1

    return class_activations, class_counts
