...
class Graph:
    ...
    def from_pt(cls, pt_path: str) -> 'Graph':
        ...
        The file should contain a dict with the following items -
            1. 'cfg': Configuration dictionary, containing similar values to a TLens configuration object.
            2. 'src_noades': Dict[str, bool] which maps a node name (i.e. 'm11' or 'a0.h11') to a boolean value, indicating if the node is part of the circuit.
    ...
    def to_graphviz(
        ...
        """
        g = pgv.AGraph(directed=True, bgcolor="white", overlap="false", splines="true", layout=layout,rankdir="LR")
        ...
        for node in self.nodes.values():
            if node.in_graph and not node.name.startswith("__layer_anchor"):
                g.add_node(
                    node.name,
        ...
        # --- Add rank anchors and layer subgraphs ---
        grouped_by_layer = defaultdict(list)
        for name, layer in layer_map.items():
            grouped_by_layer[layer].append(name)

        sorted_layers = sorted(grouped_by_layer.keys())
        anchor_nodes = []

        for layer in sorted_layers:
            node_names = [n for n in grouped_by_layer[layer] if self.nodes[n].in_graph]
            if not node_names:
                continue  # skip empty layers

            anchor = f"__layer_anchor_{layer}"
            anchor_nodes.append(anchor)
            g.add_node(anchor, style="invis", shape="point", width=0, height=0, label="")

            with g.subgraph(name=f"cluster_layer_{layer}") as sg:
                sg.graph_attr.update({
                    "rank": "same",  # Align nodes horizontally
                    "style": "invis",  # Hides the border box
                    "color": "none",  # Prevents any default stroke color
                    "label": "",  # Removes any cluster label
                })
                sg.add_node(anchor)
                for node_name in node_names:
                    sg.add_node(node_name)

        # --- Enforce top-down layer ordering using invisible edges between anchors ---
        for i in range(len(anchor_nodes) - 1):
            g.add_edge(anchor_nodes[i], anchor_nodes[i + 1], style="invis", weight=1000)
    ...
    def to_graphviz_with_auc(
        ...
        """Export the graph with AUC labels on nodes, laid out left-to-right, hiding dummy anchors"""
        g = pgv.AGraph(
            directed=True,
            bgcolor="white",
            overlap="false",
        ...
        for node in self.nodes.values():
            if node.in_graph and not node.name.startswith("__layer_anchor"):
                label = node.name
                if auc_bird or auc_bg:
                    auc_strs = []
                    if auc_bird and node.name in auc_bird:
                        auc_strs.append(f"Bird AUC={auc_bird[node.name]:.2f}")
                    if auc_bg and node.name in auc_bg:
                        auc_strs.append(f"BG AUC={auc_bg[node.name]:.2f}")
                    if auc_strs:
                        label += "\n" + "\n".join(auc_strs)
        ...
        # Layer-based alignment with invisible anchors
        ...
        for layer in sorted(layers.keys()):
            ...
            # Add invisible edge to enforce left-to-right layer ordering
            if prev_anchor is not None:
                g.add_edge(prev_anchor, anchor, style="invis", weight=100)
            prev_anchor = anchor
        ...
        for edge in self.edges.values():
            if edge.in_graph:
                ...
                if edge_labels and edge.child.name == "logits":
                    src = edge.parent.name
                    edge_kwargs["label"] = edge_labels[(src, "logits")]
        ...
        g.draw(filename, prog="dot", format="pdf")
