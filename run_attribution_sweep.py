...
if __name__ == "__main__":
    csv_path = "/home/yxpengcs/PycharmProjects/vit-spurious-robustness/output/camelyon17-set2_sweep_results.csv"
    import pandas as pd
    ...
    # task = 'fmow-mean-id'
    # tasks = ['camelyon17-set2-mean-id-new']
    tasks = []
        ...
        for task in tasks:
            ...
            if level == 'edge':
                perexample_scores = attribute(model, graph, dataloader, attribution_metric, method, ablation, get_perexample_scores=get_perexample_scores, intervention_dataloader=intervention_dataloader,
                          ig_steps=ig_steps, optimal_ablation_path=optimal_ablation_path, device=device, task=task, model_id=model_id, profile_one_batch=profile_one_batch)
            else:
