import torch
import sharpness_utils
import copy
import math
from functools import partial


def zero_init_delta_dict(delta_dict, rho, device):
...
def random_init_on_sphere_delta_dict(delta_dict, rho, device, **unused_kwargs):
    for param in delta_dict:
...
def random_gaussian_dict(delta_dict, rho, device):
    n_el = 0
...
def random_init_lw(delta_dict, rho, orig_param_dict, device='cuda', norm='l2', adaptive=False):
    ...
    for param in delta_dict:
        if norm == 'l2':
            ...
            delta_dict[param] = (2 * torch.rand_like(delta_dict[param], device=device) - 1)
...
def weight_ascent_step_momentum(
    ...
    sharpness_utils.zero_grad(model)
    output = model(x)
    # print(output.data)
    obj = loss_f(output, y)
    obj.backward()
...
def eval_APGD_sharpness(
        ...
        adaptive=False, version='default', norm='linf', device='cuda', **kwargs,
):
    ...
    init_fn = partial(random_init_lw, device=device, norm=norm, adaptive=adaptive),
