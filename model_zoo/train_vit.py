...
def save_model(args, model, args_str, step='final'):
    model_to_save = model.module if hasattr(model, 'module') else model
    model_checkpoint_dir = os.path.join(args.output_dir, 'new', args_str)
    checkpoint_path = os.path.join(model_checkpoint_dir,args.model_type + f"_{step}" + ".bin")
...
def train_model(args):
    ...
    exclude_keys = {"local_rank", "name", "model_arch", "output_dir", "eval_batch_size", "eval_every", }  # example fields to exclude
    args_str = "".join([f"{k}_{v}-" for k, v in vars(args).items() if k not in exclude_keys])
    # if args.model_type == 'scratch':
    #     save_model(args, model, args_str, 'start')
    # log_dir = os.path.join("logs", args_str, args.model_type)
    # os.makedirs(log_dir, exist_ok=True)
    # if args.local_rank in [-1, 0]:
    #     writer = SummaryWriter(log_dir=log_dir)
    # writer.add_text("Args", args_str, global_step=0)
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.batch_split == 0:
                ...
                if global_step % args.eval_every == 0 and args.local_rank in [-1, 0]:
                    ...
                    save_model(args, model, args_str, global_step)
        ...
        if global_step % t_total == 0:
            break
    save_model(args, model, args_str)
    if args.local_rank in [-1, 0]:
        writer.close()
